...
def fresh_venv_footprint(module: str, dist: str) -> dict[str, int | float | str | None]:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        target = str(ROOT) if dist == "xyg" or module == "xyg" else dist
...
def main() -> None:
    ...
    if args.packages:
        dist_by_module = {module: dist for module, dist in DEFAULT_TARGETS}
        targets = []
        for name in args.packages.split(","):
            ...
            if not name:
                continue
            if name == "xyg":
                targets.append(("xyg", "xyg"))
            else:
                targets.append((name, dist_by_module.get(name, name)))
    else:
