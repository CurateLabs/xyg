...
def bench(lib, n: int) -> dict:
    ...
    t = timeit(lambda: lib.xyg_encode_f32(_ptr(x, D), n, x[n // 2], 1.0, _ptr(enc, F)))
    encode_mpts = n / t / 1e6
    ...
    t = timeit(
        lambda: lib.xyg_m4_indices(_ptr(x, D), _ptr(y, D), n, x0, x1, buckets, _ptr(idx, U32))
    )
    zoom_ms = t * 1e3
    ...
    t = timeit(
        lambda: lib.xyg_histogram_uniform(_ptr(x, D), n, 0.0, float(n), bins, 0, _ptr(counts, D))
    )
    hist_mpts = n / t / 1e6
    hist_ms = t * 1e3
    ...
    t = timeit(lambda: lib.xyg_normalize_f32(_ptr(y, D), n, -1.0, 1.0, 0, _ptr(norm, F)))
    norm_mpts = n / t / 1e6

    # range_indices: rectangular viewport/selection scan used by drilldown.
    sel = array("I", bytes(4 * n))
    t = timeit(
        lambda: lib.xyg_range_indices(
    ...
    t = timeit(
        lambda: lib.xyg_local_log_density(
            _ptr(x, D),
            _ptr(y, D),
            local_n,
            0.0,
