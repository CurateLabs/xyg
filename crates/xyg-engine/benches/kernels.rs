...
fn scene_v9_primary_legend_consumers(bencher: Bencher, entries: usize) {
    // Keep the maximum-entry fixture inside the same Rust-owned bounds as a
    // product Scene: frame padding + title row + entry rows + plot margins +
    // the containment inset enforced by `resolved_legend_bounds`.
    let viewport_height = (116.0 + entries as f64 * 17.0).max(600.0);
    let layout = PlotLayout::new(800.0, viewport_height, 60.0, 20.0, 20.0, 50.0).unwrap();
    let sx = AxisScale::new(ScaleKind::Linear, 0.0, 1.0, 60.0, 780.0, 1.0, false).unwrap();
    let sy = AxisScale::new(
        ScaleKind::Linear,
        0.0,
        1.0,
