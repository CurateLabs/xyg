...
mod tests {
    ...
    #[test]
    fn hexbin_applies_fill_opacity_to_resolved_fill() {
        let (mut head, payload) = prefix("hexbin", 0, 0.6, "");
        head[24..32].copy_from_slice(&0.5f64.to_le_bytes());
    ...
    #[test]
    fn triangle_mesh_applies_fill_opacity_to_resolved_fill() {
        let (mut head, payload) = prefix("triangle_mesh", 0, 0.6, "");
        head[24..32].copy_from_slice(&0.5f64.to_le_bytes());
        let mut facts = Vec::new();
        facts.extend_from_slice(XYTC_MAGIC);
        facts.extend_from_slice(&XYTC_VERSION.to_le_bytes());
    ...
    #[test]
    fn violin_rejects_non_unit_fill_opacity() {
        let (mut head, payload) = prefix("violin", 0, 1.0, "");
        head[24..32].copy_from_slice(&1.5f64.to_le_bytes());
        let mut facts = Vec::new();
        facts.extend_from_slice(XYTC_MAGIC);
        facts.extend_from_slice(&XYTC_VERSION.to_le_bytes());
        facts.extend_from_slice(&1u32.to_le_bytes());
        ...
        facts.extend_from_slice(&payload);
        let err = pack_trace_compile(&facts).unwrap_err();
        assert_eq!(err.code, TraceCompileCode::OpacityChannel);
    }
}
