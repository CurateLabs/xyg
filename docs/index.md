...
- **Fast, even with lots of data.** XYG draws the detail you can actually see
...
~~~md alert warning
**Early alpha.** XYG is pre-1.0. The declarative composition model is stabilizing, but callback
...
- [Install XYG](/docs/xy/overview/installation/) and build
...
control. XYG is built for the workflows where that trade-off bites. Compose
...
The numbers back this up. In the recorded live interactive sweep, XYG reached a
...
Above 200,000 rows, XYG's default path switches to a density view while keeping
