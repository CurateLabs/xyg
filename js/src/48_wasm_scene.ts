...
function compilePainter(painter: ArrayBuffer) {
  ...
  if (colorbarLength) {
    ...
    if (flags > 7 || count < 2 || count > 16 || tickCount > 32 || titleLength > 4096 || !(lo < hi) || geometry + 24 !== end || String.fromCharCode(...bytes.subarray(geometry, geometry + 4)) !== "XYRG" || u32(geometry + 4) !== 1) throw new XygWasmError("XYG_WASM_MALFORMED_OUTPUT", "Rust painter colorbar payload is invalid");
    ...
    colorbar = { domain: [lo, hi], orientation: flags & 1 ? "horizontal" : "vertical", placement: "scene", levels: count - 1, boundaries: stops.map((stop) => stop.value), band_colors: stops.slice(0, -1).map((stop) => { const match = /rgba\((\d+) (\d+) (\d+) \/ /.exec(stop.color)!; return [Number(match[1]), Number(match[2]), Number(match[3])]; }), ticks: Array.from({length: tickCount}, (_, index) => f64(tableEnd + index * 8)), minor_ticks: !!(flags & 4), label: title || null, resolved: { bounds: [f32(geometry + 8), f32(geometry + 12), f32(geometry + 16), f32(geometry + 20)] } };
  }
