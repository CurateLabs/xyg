...
export class ChunkedColumns {
  ...
  read([x0, x1], { yRange = null, budgetBytes = 64 << 20, generation = 0 } = {}) {
    ...
    if (written === BigInt("18446744073709551615")) {
      const reason = ({ 1: "I/O failure", 2: "corrupt artifact", 3: "invalid viewport bounds", 4: "read budget exceeded", 5: "cancelled by newer viewport" })[Number(stats[5])] ?? "invalid request";
  ...
  cancelBefore(generation) { if (xyChunkedColumnsCancelBefore(this.handle, BigInt(generation)) !== 1) throw new Error("stale chunked-column handle"); }
