...
/** Versioned Rust-owned default categorical palette (ABI 360). */
export function defaultPaletteContract() {
  const version = Number(xyDefaultPaletteVersion());
  const rows = Number(xyDefaultPaletteRows());
  const textLength = Number(xyDefaultPaletteUtf8(null, 0n));
  const rgbaLength = Number(xyDefaultPaletteRgba8(null, 0n));
  if (!Number.isSafeInteger(rows) || rows <= 0 || textLength !== rows * 7 || rgbaLength !== rows * 4) {
    throw new Error("native default palette contract is malformed");
  }
  const text = new Uint8Array(textLength), rgba = new Uint8Array(rgbaLength);
  if (Number(xyDefaultPaletteUtf8(pointer(text, "uint8_t *"), BigInt(text.length))) !== text.length
      || Number(xyDefaultPaletteRgba8(pointer(rgba, "uint8_t *"), BigInt(rgba.length))) !== rgba.length) {
    throw new Error("native default palette contract copy failed");
  }
  const colors = [];
  const decoder = new TextDecoder("ascii", { fatal: true });
  for (let index = 0; index < rows; index += 1) {
    const color = decoder.decode(text.subarray(index * 7, index * 7 + 7));
    if (!/^#[0-9a-f]{6}$/.test(color)) throw new Error("native default palette text row is malformed");
    const expected = [1, 3, 5].map((offset) => Number.parseInt(color.slice(offset, offset + 2), 16));
    if (expected.some((channel, channelIndex) => rgba[index * 4 + channelIndex] !== channel)
        || rgba[index * 4 + 3] !== 255) {
      throw new Error("native default palette text/RGBA rows disagree");
    }
    colors.push(color);
  }
  return Object.freeze({ version, colors: Object.freeze(colors), rgba });
}

export const DEFAULT_PALETTE_CONTRACT = defaultPaletteContract();
export const DEFAULT_PALETTE = DEFAULT_PALETTE_CONTRACT.colors;
export const DEFAULT_MARK_COLOR = DEFAULT_PALETTE[0];
