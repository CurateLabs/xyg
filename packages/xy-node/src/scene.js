...
function packMarkerBlob(value) {
  return sceneMarkerBlobPack(value);
}
...
export function packXyTcColorChannel(trace) {
  ...
  const hasConstant = channel.constant != null ? 1 : 0;
  const flags = sceneXytcColorChannelPack(1, hasConstant);
...
function xytaHexbinPlaneObservations(trace) {
  const channel = trace.color_ch;
  if (channel == null) return { hexbinColormapPlane: false, hexbinRgbaPlaneReady: false };
