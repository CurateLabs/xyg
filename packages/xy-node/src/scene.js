...
export function channelConstantCss(channel) {
  ...
  return sceneChannelConstantCss(channel.mode, channel.constant != null, channel.constant);
}
...
export function channelEndRgba8(channel, n, fallback) {
  ...
  if (typeof channel === "object" && !Array.isArray(channel) && !ArrayBuffer.isView(channel)) {
    ...
    if (channel.mode === "categorical") {
      if (channel.codes == null) return null;
      try {
...
function gradientSolidCss(gradient) {
  ...
  for (const [, rgba] of gradient.stops) {
    packed.push(Number(rgba[0]), Number(rgba[1]), Number(rgba[2]), Number(rgba[3]));
  }
  return sceneGradientSolidCss(packed) ?? "rgb(0,0,0)";
}
...
export function polarLayout(thetaAxis = {}, rAxis = {}, plot = {}) {
  ...
  const written = xyPolarLayout(
    ...
    polarThetaDirection(polarAxisThetaDirection(thetaAxis)),
    Number(sector[0]),
    Number(sector[1]),
    categories.length,
    Number(rLo),
...
function exportColumnFinite(column) {
  ...
  return sceneFiniteAll(column);
}

function exportArraysEqual(left, right) {
  if (left == null || right == null) return false;
  return sceneArraysEqual(left, right);
}
...
function packGradientSpec(fill) {
  return sceneGradientSpecPack(fill?.space, fill?.dir, fill?.stops);
}
...
/** Chrome axis tick-label sides. Python `_pack_chrome_axis` reads `options.get("tick_label_sides")` only. */
export function chromeAxisTickLabelSides(options) {
  return (options ?? {}).tick_label_sides;
}
