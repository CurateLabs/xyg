...
test("Node projects Rust-owned Scene support decisions verbatim", () => {
  ...
  assert.equal(new DataView(polarScene.buffer, polarScene.byteOffset).getUint32(4, true), 28);
  ...
  assert.equal(new DataView(polarBarScene.buffer, polarBarScene.byteOffset).getUint32(4, true), 28);
  ...
  assert.equal(new DataView(polarContourScene.buffer, polarContourScene.byteOffset).getUint32(4, true), 28);
...
test("Node Scene v28 compiles constant dash polylines and keeps curves fail-closed", () => {
  ...
  assert.equal(new DataView(scene.buffer, scene.byteOffset).getUint32(4, true), 28);
  ...
  assert.throws(() => curved.toScene(), /curved, non-round, or authored markers/);
});
...
test("Node public Figure matches the combined Python authored Scene v25 fixture", () => {
  ...
  const scene = figure.toScene();
  assert.equal(new DataView(scene.buffer, scene.byteOffset, scene.byteLength).getUint32(4, true), 28);
...
test("Node Scene compiles column and histogram as Rect records", () => {
  ...
  assert.equal(new DataView(columnScene.buffer, columnScene.byteOffset).getUint32(4, true), 28);
