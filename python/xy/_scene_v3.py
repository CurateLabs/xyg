"""Thin figure-to-Scene v3 compiler for the migrated core-mark subset.
...
def _constant_color(trace: Any, fallback: str) -> str:
    ...
    if channel.mode != "constant" or channel.constant is None:
        raise UnsupportedSceneV3("Scene v3 does not yet support data-driven paint channels")
...
def figure_scene(
    ...
    *,
    ...
    """Compile representative scatter/line/bar plus x/y axes to Scene v3."""
    ...
    if figure.colorbar_options or figure.extra_legends:
        raise UnsupportedSceneV3("Scene v3 does not yet encode colorbars or extra legends")
    ...
    if unsupported is not None:
        raise UnsupportedSceneV3(f"Scene v3 figure compilation does not yet support {unsupported}")
    ...
    for trace in figure.traces:
        ...
        if trace.hidden or trace.has_per_item_channels():
            raise UnsupportedSceneV3("Scene v3 does not yet encode hidden or per-item styled marks")
        style = trace.style
        if any(key in style for key in ("dash", "curve", "linecap", "marker_path", "marker_glyph")):
            raise UnsupportedSceneV3(
                "Scene v3 does not yet encode dashed, curved, or authored markers"
            )
        ...
        if trace.kind == "bar":
            if any(value is None for value in (trace.x0, trace.y0, trace.x1, trace.y1)):
                raise ValueError("bar Scene v3 compilation requires four rectangle columns")
