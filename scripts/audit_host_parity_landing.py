...
Stdlib-only orchestrator. Proves the branch meets the inventory + cross-host +
ABI smoke bar documented in ``spec/process/m2-close.md`` without running the
full pytest suite or browser smokes.
...
CROSS_HOST_TESTS = sorted(ROOT.glob("tests/test_*cross_host*.py"))
...
STEPS: tuple[tuple[list[str], str], ...] = (
    ...
    (
        ...
        "audit + WASM tick contract tests",
    ),
    (
        [
            "uv",
