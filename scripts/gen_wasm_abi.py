...
def render_python_aggregate(manifest: dict[str, object]) -> str:
    """Emit the small Python-facing part of the browser aggregate contract.

    The split Python payload advertises the same source capacity as the Rust/
    WASM transport.  Keeping this generated avoids a second handwritten limit
    in the public host.
    """
    aggregate = manifest["aggregate"]
    ...
    return "\n".join(
        [
            ...
            "# Generated by scripts/gen_wasm_abi.py from spec/wasm/abi.json.",
            "",
            f"WASM_AGGREGATE_MAX_POINTS = {int(aggregate['max_points'])}",
            f"WASM_AGGREGATE_STREAM_CHUNK_POINTS = {int(aggregate['stream_chunk_points'])}",
            "",
        ]
    )
...
def main() -> int:
    ...
    expected_python_aggregate = render_python_aggregate(manifest)
    if args.check:
        ...
        actual_python_aggregate = (
            PYTHON_AGGREGATE_OUTPUT.read_text(encoding="utf-8")
            if PYTHON_AGGREGATE_OUTPUT.exists()
            else ""
        )
        if actual_python_aggregate != expected_python_aggregate:
            raise SystemExit(
                f"generated Python aggregate contract is stale: run {Path(__file__).name}"
            )
    else:
        OUTPUT.write_text(expected, encoding="utf-8")
        RUST_TYPED_SERIES_OUTPUT.write_text(expected_rust, encoding="utf-8")
        PYTHON_AGGREGATE_OUTPUT.write_text(expected_python_aggregate, encoding="utf-8")
    return 0
