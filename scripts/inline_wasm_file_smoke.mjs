...
try {
  worker.postMessage({type:"init",requestId:1,base64:globalThis.__xygInlineWasm.base64,expectedAbiVersion:21,expectedSceneVersion:23,maxArenaBytes:1048576});
  const ready=await wait(worker,1);if(!ready.ok||ready.value?.abiVersion!==21)fail("classic worker init/diagnostics failed");
