...
**Install doors.** Python: `pip install xyg` / `import xyg`. JavaScript /
browser: `npm install @curatelabs/xyg`.
...
- [`dashboard-compositor.md`](design/dashboard-compositor.md) — the shared
  WebGL host and Rust-owned cross-chart derived-resource admission contract.
...
The `xyg.pyplot` shim and its corpus-defined compatibility contract.

- [`compat.md`](matplotlib/compat.md) — the supported `xyg.pyplot` surface and
