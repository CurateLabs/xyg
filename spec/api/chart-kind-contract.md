...
colors, §36). The registry and `markOf()` are exported (`xyg.MARK_KINDS`)
...
primitives does not have to pay it. `xyg.register_mark` (`python/xyg/plugins.py`,
...
3. Public composition surface: a `xyg.<K>(...)` mark factory, an entry in
