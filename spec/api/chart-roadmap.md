...
| 5 | Histogram | count, probability, density, cumulative histogram | Implemented core | Both composition hosts use Rust-owned automatic edge resolution and uniform binning plus the shared rectangle renderer; `cumulative=True` (count CDF and, with `density=True`, empirical CDF) is implemented. Follow-up: viewport-aware re-binning for huge streamed distributions. |
...
|---:|---|---|---|---|
| 1 | Histogram | Core statistical chart in Plotly, Matplotlib, Altair; common first chart for distributions. | Implemented core: Rust-owned automatic edge resolution and uniform binning + shared instanced rectangle renderer, incl. `density` and `cumulative` modes. Follow-up: viewport-aware re-binning for very large streamed distributions. | `xyg.histogram_chart(xyg.hist(values, bins=512, cumulative=True))` |
