...
This is the single chart-type roadmap for xyg. The destination is every chart
...
| Histogram | Implemented core | Rust-owned automatic edge resolution and uniform binning plus the shared rectangle renderer. |
...
| 3 | Bar / column | vertical bar, horizontal bar, grouped, stacked, normalized stacked, diverging bar | Implemented core | `xyg.bar(...)` / `xyg.column(...)` ship categorical/numeric vertical and horizontal bars, grouped bars, stacked bars, and normalized stacked bars (`mode="normalized"`) through the shared rectangle renderer. Follow-up: labels. |
| 4 | Area | filled line, stacked area, streamgraph, ridgeline-lite area bands | Implemented core | `xyg.area(...)` ships a filled area with scalar/array baseline and optional line overlay. Follow-ups: stacked area helpers and streamgraph offsets. |
| 5 | Histogram | count, probability, density, cumulative histogram | Implemented core | Both composition hosts use Rust-owned automatic/authored edge resolution, counting, density, and cumulative assembly plus the shared rectangle renderer; `cumulative=True` (count CDF and, with `density=True`, empirical CDF) is implemented. Follow-up: viewport-aware re-binning for huge streamed distributions. |
| 6 | Pie / donut | pie, donut, nested donut, variable-radius pie | Implemented | `xyg.pie_chart(labels, values)` composes polar wedge bars (hole/pad/rounded corners) and owns its tooltip — a slice reads category, value, share; theta/rim never show. `xyg.pyplot` keeps its Matplotlib-style pie. Nested/variable-radius remain future depth. |
| 7 | Heatmap / image / matrix | heatmap, image, annotated matrix, correlation matrix, cohort heatmap | Implemented core | `xyg.heatmap(...)` renders matrix cells through a compact grid texture with continuous colormaps and categorical/numeric axes. Native static export borrows canonical f64 spans and normalizes only sampled pixels in Rust, verified through 4.29B cells without a derived grid or RGBA expansion. Follow-ups: annotation and tiled huge-image browser transport. |
...
| 9 | Candlestick / OHLC | candlestick, OHLC bars, volume overlay, range selector | Prototyped (PR closed unmerged) | `xyg.candlestick(...)`/`xyg.ohlc(...)` + `xyg.candlestick_chart(...)` on the closed `codex/finance-charting-surface` exploration branch: OHLC decimation, shared-y f32 frame, time axes, hover, and a volume pane. Critical finance surface; inherits LOD and time-axis work from core primitives. |
...
| 18 | Radar / polar | radar, spider, polar area, radial bar, polar heatmap/contour | Implemented core | `xyg.polar_chart` renders the allowlisted line/scatter/area/bar/column/heatmap/contour/errorbar schemas through the polar coordinate system (spec/design/polar-axes.md); `xyg.radar_chart`, `xyg.polar_bar_chart` and `xyg.wind_rose` are the compositions. Sector layout, hole/r-origin, categorical θ, log/symlog radius, and polygonal grids ship. Follow-ups are rule/band geometry, polar LOD, facets/animation, and angular navigation/selection. |
...
| 20 | Small multiples | facet grid, repeat chart, trellis chart, pair grid | Implemented core | `xyg.facet_chart(...)` builds per-panel screen-bounded Figures with optional shared domains. |
...
| 30 | Sankey / alluvial | Sankey, alluvial, dependency wheel | Implemented core | `xyg.sankey_chart(links)` — Rust layout (`xy_sankey_layout` / `_sankey.compute_layout`: layering, barycentre crossing minimisation, endpoint stacking) over the `ribbon` primitive (per-end colours, flow-axis gradient, protocol v11). Alluvial/dependency-wheel remain compositions to build on the same primitive. Deferred: GPU picking (CPU hover only), legend swatches, cycle auto-breaking. |
| 31 | Network/tree/org | network graph, force graph, tree, dendrogram, org chart, arc diagram | In progress | Core feature; design: `spec/design/graph-mark.md`. Requirements: `graph-fork-requirements.md`; dual-host: `host-parity.md`. Algorithms stay in GraphForge. |
| 32 | Scientific vector fields | quiver, barbs, streamplot, wind rose | Implemented across core and `xyg.pyplot` | Quiver, barbs, and bounded streamlines feed shared instanced segments through `xyg.pyplot`; `xyg.wind_rose(...)` bins compass bearings and speed bands into stacked polar sectors in core. |
| 33 | Irregular grid science | pcolormesh, tricontour, tripcolor, triangular mesh | Implemented in `xyg.pyplot` | Curvilinear quads and explicit/native triangulations route through indexed meshes and marching-triangle kernels. |
...
| 40 | Tables and table-adjacent views | table, pivot-like summary, annotated table | Partially implemented in `xyg.pyplot` | `table(cellText=, cellColours=, cellLoc=, colWidths=, rowLabels=, colLabels=, loc=, bbox=)` tessellates cells into `triangle_mesh` geometry with per-cell colors, rules, and text, so it renders through the core mark path rather than as dashboard DOM. Pivot-like summaries and annotated-table depth remain deferred. |

### P1 - Add next

| Rank | Chart | Why it is popular | Why it fits xy | Suggested API |
|---:|---|---|---|---|
| 1 | Histogram | Core statistical chart in Plotly, Matplotlib, Altair; common first chart for distributions. | Implemented core: Rust-owned automatic/authored edge resolution, counting, density, and cumulative assembly + shared instanced rectangle renderer. Follow-up: viewport-aware re-binning for very large streamed distributions. | `xyg.histogram_chart(xyg.hist(values, bins=512, cumulative=True))` |
| 2 | Bar / column | Present in every major library; expected for categorical comparison. | Implemented core: category axis + shared instanced rectangle renderer for basic, grouped, stacked, normalized stacked, and horizontal bars. Follow-up: labels. | `xyg.bar_chart(xyg.bar(x, y))` |
| 3 | Area / filled line | Common extension of line charts in Plotly, Chart.js, Highcharts, Altair. | Implemented core: sorted x, M4 first payload, and filled WebGL segment strips. Follow-up: stacked area helper. | `xyg.area_chart(xyg.area(x, y))` |
| 4 | Heatmap / image | Common in scientific and BI tools; Matplotlib, Plotly, Altair, Highcharts all surface it. | Implemented core: matrix-to-grid texture path with color channel reuse. Follow-up: image/raster tiling for huge grids. | `xyg.heatmap_chart(xyg.heatmap(z, x=None, y=None))` |
...
| 14 | Pie / donut | Very popular in basic chart libraries and user expectations. | Implemented: `xyg.pie_chart` in the composition API + `xyg.pyplot`. |
...
| 23 | Sankey / alluvial / dependency wheel | Common flow visualization in BI and systems analysis. | Core implemented: `xyg.sankey_chart` + `ribbon`. Interaction depth (link picking, hover highlight) tracked in the ribbon contract's deferred list. |
| 24 | Network / tree / org / dendrogram / arc | Graph and hierarchy compatibility. | Core graph viz feature. Requirements: `spec/design/graph-fork-requirements.md`; dual-host: `spec/design/host-parity.md`. |
| 25 | Gauge / bullet / indicator | Dashboard/KPI compatibility. | Mostly chrome and layout, not large-array rendering. |
| 26 | Table-adjacent views | Table, pivot-like summary, annotated table. | Basic `table` is implemented through `xyg.pyplot` as tessellated cell geometry; pivot-like summary and annotated-table depth remain deferred. |
...
| 29 | Quiver / barbs / streamplot / wind rose | Scientific and engineering vector fields. | Quiver, barbs, and streamplot are implemented through `xyg.pyplot`; `xyg.wind_rose(...)` ships as a core helper over stacked polar bars. |
| 30 | Pcolormesh / tricontour / tripcolor | Matplotlib-style irregular grid science. | Implemented through `xyg.pyplot` with native quad/triangle geometry. |
...
active state via `--chart-modebar-active`) is `xyg.CHART_DOM_SLOTS`.
...
2. **Reflex-first reactive API — shipped** (PR #55, `python/reflex_xy/`; see
   ...
   interactions shipped in PR #105: `xyg.facet_chart(...)` takes
   ...
   pan/zoom controls shipped in PR #103: `xyg.interaction_config(...)` exposes
...
4. **Pie / donut depth.** `xyg.pie_chart(labels, values)` ships in the
   composition API over unequal-width polar bars, and `xyg.pyplot` supplies the
...
# shipped
xyg.histogram_chart(xyg.hist(values, bins=512, density=False, cumulative=False))
xyg.bar_chart(xyg.bar(categories, values, mode="grouped", orientation="vertical"))
xyg.area_chart(xyg.area(x, y, base=0.0))     # + fill=linear-gradient, curve, dash
xyg.heatmap_chart(xyg.heatmap(z, x=None, y=None, colormap="viridis"))
xyg.box_chart(xyg.box(values, group=None))
xyg.violin_chart(xyg.violin(values, group=None, bandwidth="auto"))
xyg.chart(xyg.errorbar(x, y, yerr=..., xerr=...))
xyg.chart(xyg.hexbin(x, y, gridsize=50, color_scale="log"))
xyg.candlestick_chart(xyg.candlestick(x, open, high, low, close))  # prototyped (closed finance PR)
xyg.pie_chart(labels, values, hole=0.55)    # wedge bars + an owned slice tooltip
```

New chart kinds land as composition marks plus a family container
(`xyg.box_chart(xyg.box(...))`, `xyg.histogram_chart(xyg.hist(...))`, …).
