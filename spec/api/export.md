...
| SVG | `_svg.to_svg`; explicit `Figure.to_scene()` output can be passed to the Rust Scene v3 SVG consumer during migration | none — SVG is native-only |
| PDF | `_svg.to_svg` → `_pdf.svg_to_pdf`; Scene v3 remains explicit until it carries ticks/text/chrome | `Page.printToPDF` |
