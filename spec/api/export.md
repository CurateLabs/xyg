...
| SVG | Supported public literal Cartesian scatter/line/ordinary-rect exports use Rust Scene SVG, including the bounded primary annotation family listed for PNG. `FacetGrid` applies that same route independently to each supported, no-background-override panel and namespaces the closed Scene clip-id vocabulary before nested composition. `_svg.to_svg` remains the compatibility backend for every other panel. | none — SVG is native-only |
| PDF | Supported public literal Cartesian scatter/line/ordinary-rect exports consume Rust Scene SVG through `_pdf.svg_to_pdf`, including the bounded primary annotation family; this includes independently supported `FacetGrid` panels. The compatibility path is `_svg.to_svg` → `_pdf.svg_to_pdf`. | `Page.printToPDF` |
