...
| PDF | Supported public Cartesian circle-scatter exports consume Rust Scene SVG through `_pdf.svg_to_pdf`, including the bounded primary annotation family; this includes independently supported `FacetGrid` panels. The compatibility path is `_svg.to_svg` → `_pdf.svg_to_pdf`. | `Page.printToPDF` |
