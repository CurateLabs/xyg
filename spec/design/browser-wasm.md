...
`WASM_ABI_VERSION` is 21. ABI 21 retains the bounded `XYSA` v1 envelope and
...
`SCENE_VERSION` remains independently versioned and is 20 for this contract.
...
Scene v20 retains record metadata byte 3 explicitly: `0` retains legacy
...
Scene v20 (`SCENE_VERSION = 20`), a 300-byte header, 64-byte trace descriptors, viewport/plot f32
bounds, bounded trace and tick counts, and absolute offsets to the tick and
UTF-8 label tables. Header bytes 64–263 are the exact validated Scene v20
