...
and Node cannot drift on first paint or `decimate_view`. Node `_emitLine`
and `_emitArea` pass `_binningCoords` into M4 like Python `_m4_decimate`;
cross-host proof: `tests/fixtures/payload_cross_host.json` case
`line_log_m4_bin_x`. Node `_emitRibbon` ships `color2_ch` on registry key
`color_target` like Python `_emit_ribbon`; cross-host proof:
`tests/fixtures/payload_cross_host.json` case `ribbon_color2_ch`. Node
`_emitHexbin` ships `color_ch` via payload channel attach like Python
`_emit_hexbin`; cross-host proof: `hexbin_colormap` (no metric column).
Hosts still gather extra columns and ship the chosen rows.
...
ABI 323 observation blobs remain bounded to 4 MiB. Rust budgets the complete
ABI 318 XYTA record (including its fixed prefix and every non-source sidecar),
then admits density f64 x/y only when that whole record fits. Python and Node
marshal identical observations without duplicating a row cutoff; when Rust
omits the planes, the explicit static direct-geometry fallback remains. The
four-tier authored-Scene evidence
explicitly sets the shared density tri-state false (`density=False` /
`force_density: false`) so its intended 1M direct workload cannot be changed
by Node's automatic interactive tier heuristic. The Python and Node generators
must remain byte-identical at 100, 10k, 100k, and 1M records.
...
extents both hosts already had. Consequently, ordinary autoranged line,
scatter, bar, and histogram figures now produce byte-identical Scene, SVG, and
raster-command output in Python and Node; the executable differential is
`tests/test_static_export_cross_host.py`. Unsupported browser CSS remains an
explicit preflight miss and only Python's documented compatibility renderer
handles it. ABI 107 `xyg_scene_resolve_mark_styles` /
