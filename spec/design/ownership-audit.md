...
| `js/src/51_annotations.ts` | Shared TypeScript browser client | `browser-client` | `literal-projection-only`; Scene v17 owns rule/band/marker geometry, order, clipping, defaults, bounded attached-label anchors, and literal Cartesian straight-arrow projection/head geometry in Rust; callouts and rich annotation forms remain migration debt | #116 |
