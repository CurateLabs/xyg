...
1. `_payload` emit orchestration — role-block expansion, extra-column gather, and ship (index math is ABI 204/205; count budget is ABI 214)
...
| `python/xyg/_payload.py` | Python host with canonical-policy debt | `python-scene-migration` | `split-and-move-rust`; ABI 122 owns compile-time payload LOD and the visible-row mask; ABI 204 owns line M4 emit indices; ABI 205 owns remaining emit visible/even/sample indices; ABI 214 owns the stem/errorbar count budget; emitters still expand role blocks, gather extra columns, and ship | #58 |
