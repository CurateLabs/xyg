...
| Check | Result |
| --- | --- |
| `verify_ownership.py` | 297 tracked production files classified |
| `abi_smoke.py` | 557 ABI checks passed |
| `audit_python_host_core.py` | 7 `python-scene-migration` files; ~9.9k lines; 127 delegate hooks through ABI 357 category label kinds |
...
Largest remaining Python core surface after materialization retirement: secondary §302 compat and composition (`_svg.py`/`_raster.py` path assembly, `lod.py` cache wiring). `marks.py`, `_figure.py`, and `channels.py` are keep-host re-export hubs over `_marks_*.py`, `_figure_*.py`, and `_channels_*.py`. Payload emit and Scene pack product paths are marshal-only on the #853 branch.
...
3. `lod.py` viewport/sample cache wiring (offset/scale meta is ABI 255; offset math is ABI 208).
