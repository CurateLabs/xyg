...
projection and annular-sector tessellation, polar errorbar polylines, clip, rings/spokes, and rim tick labels in `scene.rs` + `polar.rs`.
Python `_PolarProjection` remains a thin packer plus wedge/ring/polygon helpers
for the compatibility exporters. Polar heatmap/contour stay
