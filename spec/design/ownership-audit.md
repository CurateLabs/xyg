...
Top remaining line counts (secondary section 302): `_svg.py` ~6220, `_raster.py` ~3490, `marks.py` 3340, `_figure.py` 2488, `channels.py` 1231, `_paint.py` ~640 (shared static-export paint/geometry layer).
...
- `_paint.py` — shared static-export paint/geometry layer (fill/stroke bundles, scatter/ribbon helpers, ABI 210/211/212 `hexbin_ring`/`step_arrays`/`authored_marker_points`, `polar_clip_line_segments`, CSS→RGBA8); ABI 206 `effective_rgba` marshal; `triangle_mesh_boundary` recorded stay-host joined-fill walk
