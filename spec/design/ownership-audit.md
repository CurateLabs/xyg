...
## Current-tree re-audit (after leftover #283 / ABI 210)
...
1. Compat marker paths and step/stairs expand in `_svg.py` / `_raster.py`
...
ABI 209 `xyg_polar_wedge_points` owns compatibility annular-sector flatten. ABI 210 `xyg_hexbin_ring` owns pointy-top hexagon vertex offsets scaled by cell pitch. ChartView `_buildHexbinMark` keeps the same fractions until WASM. Next kernel: compat marker paths and step/stairs expand. Do not wrap Scene's RGBA-grid raster as a compat path.
...
| `python/xyg/_raster.py` | Python host with canonical-policy debt | `python-scene-migration` | `split-and-move-rust`; ABI 121 tessellation via `kernels` directly (#310); ABI 206 owns remaining `_lut` / linear density / effective rgba (#313); ABI 210 owns hexbin ring offsets; `triangle_mesh_boundary` stays host geometry | #58 |
...
| `python/xyg/_svg.py` | Python host with canonical-policy debt | `python-scene-migration` | `split-and-move-rust`; ABI 207 owns polar heatmap inverse-map hits; ABI 209 owns polar wedge flatten; ABI 210 owns hexbin ring offsets; hosts still color sampled cells and emit SVG `A` arcs | #58 |
