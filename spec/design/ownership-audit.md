...
Top remaining line counts (secondary section 302): `_svg.py` ~6240, `_raster.py` ~3495, `marks.py` 3340, `_figure.py` 2488, `channels.py` 1231, `_paint.py` ~610 (shared static-export paint/geometry layer).
...
- `_paint.py` — shared static-export paint layer (fill/stroke bundles, stroke/CSS metadata helpers, `ribbon_fill_rgba`, `polar_clip_line_segments`, `colormap_lut`, CSS→RGBA8, opacity); ABI 206 `effective_rgba` marshal; `triangle_mesh_boundary` recorded stay-host joined-fill walk
