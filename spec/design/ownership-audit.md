...
**M2 close tracker ([#731](https://github.com/CurateLabs/xyg/issues/731) — OPEN).** Kernel orchestration for payload emit and Scene pack is landed; children [#732](https://github.com/CurateLabs/xyg/issues/732) (gather/ship + density grid ship, ABI 310–315) and [#733](https://github.com/CurateLabs/xyg/issues/733) (scene orchestration plans, ABI 305–309) are **closed**. M2 is **not** complete: hosts still materialize rows/grids, walk field-byte pack steps, and carry recorded Node stay-host TAP diffs until serial merge finishes. Reproduce the inventory with `python3 scripts/audit_python_host_core.py`; gate with `python3 scripts/verify_ownership.py` and `python3 scripts/abi_smoke.py`.
...
Largest remaining Python core surface: `_payload` bin2d/pyramid compose and residual row materialization after plan kernels ABI 292–315 (line/area/scatter/bar-compact xy rows and density wasm_source/sample registry route through `_ship_registry_columns`; density grid buf/rgba ship and nested attach order route through `payload_density_grid_ship_plan`; channel attach routes through `channels.ship_registry_attach`). Secondary debt: `_svg.py`/`_raster.py` compat path assembly, `marks.py`/`_figure.py` composition, `channels.py` label factorization, `lod.py` cache wiring. Scene pack dispatch is kernel-owned ([#733](https://github.com/CurateLabs/xyg/issues/733) closed); `_scene_v3.py` still packs field bytes and calls orchestration plans before `xyg_scene_encode_product`.

**#731 close checklist (honest remainder — do not mark M2 complete).**
...
**Still blocks “Python is only a host”** (compatibility modules stay until these twins move or stay-host is recorded with diffs). **#731 close bar** is Node TAP + cross-host proof; items 1–2 below are post-orchestration residual host work (not reopening #732/#733):

1. `_payload` residual materialization — bin2d/pyramid compose stays host; gather/ship registry policy is kernel-owned (ABI 310–315, [#732](https://github.com/CurateLabs/xyg/issues/732) closed). Hosts still materialize line/area/scatter/bar-compact xy rows via `_ship_registry_columns`, density grids via `payload_density_grid_ship_plan`, and channel rows via `channels.ship_registry_attach`.
2. `_scene_v3.py` / Node `scene.js` field-byte pack before encode — dispatch/plan orchestration is Rust-owned (ABI 305–309, [#733](https://github.com/CurateLabs/xyg/issues/733) closed); hosts still coerce objects and walk pack steps.
