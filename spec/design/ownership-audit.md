...
| `crates/xyg-engine/src/kernels.rs` | Rust safe engine | `rust-engine` | `keep-rust`; ABI 129 Cartesian static-export grid colormap (`colormap_rgba_into` / `colormap_rgba_canonical_into`); ABI 218 Scene dash admit; ABI 219 Scene linecap admit; ABI 220 density overlay opacity; ABI 221 Scene marker-path admit | — |
...
| `packages/xy-node/src/scene.js` | Node host with canonical-policy debt | `node-scene-migration` | `split-and-move-rust`; ABI 218 owns Scene dash admit; ABI 219 owns Scene linecap admit; ABI 221 owns Scene marker-path admit; hosts still coerce list vs string and fail-close empty strings | #58 |
...
| `python/xyg/_scene_v3.py` | Python host with canonical-policy debt | `python-scene-migration` | `split-and-move-rust`; ABI 218 owns Scene dash admit; ABI 219 owns Scene linecap admit; ABI 221 owns Scene marker-path admit; hosts still coerce list vs string and fail-close empty strings | #58 |
