...
Leftover children [#287](https://github.com/CurateLabs/xyg/issues/287)–[#313](https://github.com/CurateLabs/xyg/issues/313) and parents [#271](https://github.com/CurateLabs/xyg/issues/271)–[#283](https://github.com/CurateLabs/xyg/issues/283) are closed. No `python-scene-migration` production files remain (2026-09-01 §302 hub split). Do not delete `_svg.py`, `_raster.py`, `_scene.py`, `marks.py`, `_legendfit.py`, `_payload.py`, or `_textblock.py` until Rust owns the path **and** differentials are green. Do not delete `_scene_v3.py`. Do not route pyplot through Scene.
...
| `audit_python_host_core.py` | 0 `python-scene-migration` files; 0 delegate hooks |
...
| `node-host` | 8 | `keep-host` | current owner |
...
| `packages/xy-node/src/payloadTraceMaterialize.js` | Node host | `node-scene-migration` | `keep-host`; ABI 321 trace emit materialize marshal | — |
...
| `packages/xy-node/src/color.js` | Node host with canonical-policy debt | `node-scene-migration` | `split-and-move-rust`; ABI 213 owns CSS/numeric split, domain pad, and direct RGBA admit; ABI 251 owns clip-quantize u8; `quantizeUnitU8` composes normalize then ABI 251; `factorize.js` delegates fixed-width categorical columns to native factorize/remap like Python `channels._factorize_categories`; JS string arrays still use label-policy fallback; hosts still pin palettes | #58 |
...
| `packages/xy-node/src/sceneBulkNative.js` | Node host | `node-scene-migration` | `keep-host`; koffi bindings for scene bulk packers (ABI 321-324); mirrors `_scene_bulk_native.py` | — |
