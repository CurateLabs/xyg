...
## Current-tree re-audit (after leftover #283 / ABI 208)
...
1. Compat `_svg.polar_wedge_points` / `_polar_wedge_path` — Scene already tessellates through engine `polar::polar_wedge_points`; there is no C ABI yet. Python tests pin an explicit `steps` count the Scene kernel does not take.
2. Compat hex rings, marker paths, and step/stairs expand in `_svg.py` / `_raster.py`
3. `channels.resolve_color` versus the thinner Node `color.js` (CSS/numeric split, categorical factorization, direct RGBA)
4. `_payload` emit orchestration, including the stem/errorbar budget `max(1024, px_width*4)` (index math is ABI 204/205)
5. `_scene_v3.py` / Node `scene.js` pack and figure-to-record orchestration
6. Curved annotation arrows `_arrowgeom.py` ↔ `js/src/51_annotations.ts`
7. `lod.py` remaining viewport/sample/`EncodedColumn` packing — offset/scale math is ABI 208; hosts still map `log`/`symlog` onto `pin_zero`
8. `marks.py`, `facets.py`, `_figure.py`, `_annotations.py` composition and `_fontmetrics.py` generated DejaVu table used by compatibility SVG gutters

Next kernel after ABI 208: expose `xyg_polar_wedge_points` for compatibility flatten (optional `steps`, `0` = `polar_bar_segments`). Do not wrap Scene's RGBA-grid raster as a compat path.
...
| `python/xyg/_svg.py` | Python host with canonical-policy debt | `python-scene-migration` | `split-and-move-rust`; ABI 207 owns polar heatmap inverse-map hits; hosts still color sampled cells; `polar_wedge_points` remains a Python twin of engine tessellation | #58 |
