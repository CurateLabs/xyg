...
| `crates/xyg-engine/src/tick_layout.rs` | Rust safe engine | `rust-engine` | `keep-rust`; ABI 123 tick-label collision thinning; ABI 128 authored tick-window; ABI 199 Scene product-path authored filter/pairing; ABI 130 tick-label formatting (`format_axis_tick` in `scene.rs`) | — |
