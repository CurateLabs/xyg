...
Scene compile is explicit XYPL v1. Scene v26 owns polar line/scatter/area
projection, clip, rings/spokes, and rim tick labels in `scene.rs` + `polar.rs`.
Python `_PolarProjection` remains a thin packer plus wedge/ring/polygon helpers
for the compatibility exporters. Polar bar/heatmap/contour/errorbar stay
