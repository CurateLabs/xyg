...
lattice tessellation, ABI 192 polar painted heatmap Image blit, ABI 193 heatmap/hexbin stroke_opacity, ABI 194 polar hexbin / custom reducers / categorical direct_rgba, ABI 195 triangle-mesh custom role / per-item face intern, clip, rings/spokes, and rim tick labels in `scene.rs` + `polar.rs`.
...
identity metadata). Python and Node pack six authored
...
categorical / `direct_rgba` 1×N XYHP RGBA onto HexCell PolyFills. ABI 195 admits triangle-mesh custom `role` and per-item fill/stroke/width interned from packed XYHP kind 6. LOD over the 1,024-group painter
