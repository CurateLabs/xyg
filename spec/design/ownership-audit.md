...
| `python/xyg/lod.py` | Python host with canonical-policy debt | `python-scene-migration` | `split-and-move-rust`; ABI 208 owns `geometry_offset` / `f32_safe_scale`; ABI 216 owns log-family `pin_zero` names; ABI 255 owns EncodedColumn offset/scale/kind-presence packing; ABI 326 owns `aligned_window`; `xy_lod_plan` / `xy_drill_decision` / sample kernels are Rust-owned with cross-host proof in `tests/test_lod_cross_host.py`; stay-host drill_history cache and `BufferWriter` | #58 |
