...
| Check | Result |
| --- | --- |
| `verify_ownership.py` | 291 tracked production files classified |
...
Largest remaining Python core surface after materialization retirement: secondary §302 compat and composition (`_svg.py`/`_raster.py` path assembly, `marks.py`/`_figure.py` hub, `channels.py` label factorization, `lod.py` cache wiring). Payload emit and Scene pack product paths are marshal-only on the #853 branch.
