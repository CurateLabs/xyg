...
| `packages/xy-node/src/figure.js` | Node host with canonical-policy debt | `node-scene-migration` | `split-and-move-rust`; ABI 220 owns density overlay opacity; Node scatterPerItemChannels ignores style.color_channel; hosts still default omitted opacity to `0.8` and gather/ship | #58 |
