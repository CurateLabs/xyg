...
| `packages/xy-node/src/encode.js` | Node host | `node-host` | `keep-host`; ABI 206 owns colormap LUT RGB; `colormapLutRgba8` host-packs alpha 255; ABI 254 owns `packArrowStyle` CSV pack; ChartView still parses those strings until WASM; ABI 255 owns EncodedColumn offset/scale/kind-presence packing; hosts still copy the original kind string | — |
