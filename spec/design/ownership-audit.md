...
**M2 close tracker ([#731](https://github.com/CurateLabs/xyg/issues/731) — CLOSED 2026-08-31).** Kernel orchestration (ABI 218–315) and host materialization retirement (ABI 316–325, big pushes 1–3 on PR [#853](https://github.com/CurateLabs/xyg/pull/853)) are landed on branch `cursor/m2-payload-writer-ship-7ce1` pending merge to `main`. `_payload.py` (219 lines) and `_scene_v3.py` (396 lines) are marshal-only; keep-host coercion lives in `_payload_trace_materialize.py` and `_scene_marshal.py`. Reproduce the inventory with `python3 scripts/audit_python_host_core.py`; gate with `python3 scripts/verify_ownership.py` and `python3 scripts/abi_smoke.py`.
