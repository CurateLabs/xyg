...
| Minor ticks (`minor_tick_values`, `minor_style`) | The public composition surface still refuses them because the two static exporters do not yet draw minor polar chrome. The browser ChartView does paint Rust-resolved minor angular spokes/rim tick marks and radial rings/tangent tick marks for direct/Scene input, using the minor grid/tick color, width, opacity, dash, length, and direction. This prevents an accepted direct-browser minor plane from being silently dropped without overstating cross-renderer support. `xyg.theta_axis`/`xyg.r_axis` continue to point users at `tick_values` until static parity lands. |
...
the public static renderers do with the values — and
