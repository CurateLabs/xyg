...
| statistics: `xyg_correlation`, `xyg_weighted_ecdf`, `xyg_binned_ecdf`, `xyg_histogram_cumulative`, `xyg_histogram2d`, `xyg_stacked_bounds` | Rust (ABI v101) | correct — row-scan reductions; 1D automatic histogram edge policy and uniform binning are Rust-owned while labels remain host presentation. ABI 100 binned ECDF owns finite filtering, automatic/constant domain, bounded uniform counting, all-finite-mass normalization, empty-bin compaction, right edges, and the zero anchor. ABI 101 owns cumulative count and density-width integration into the final Rect heights. Unweighted `xyg_histogram2d` fans out with per-worker u64 grids (integer merge, thread-count invariant); the weighted case stays serial because f64 accumulation order must not vary with core count (§21) |
...
coordinates with a 10,000-bin ceiling; `xyg_histogram_cumulative` ✅ ABI 101
atomic count/density integration into canonical Rect heights) · hexbin
