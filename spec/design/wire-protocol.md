...
The canonical Scene v4 batch is currently an in-process native host contract,
...
bytes as a binary attachment with explicit scene-version validation; it must
not translate the numeric records through JSON or silently accept unknown
kinds.
...
installed `xyg` distribution, repairing a stale link if the install moved.
