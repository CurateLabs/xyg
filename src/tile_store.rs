...
//!
//! No unsafe here; the C-ABI shell in lib.rs owns marshaling (engine doc
...
/// Cells per tile side (D1). 256² × u32 = 256 KiB count slabs; 4 KiB pages
/// divide every slab size, so positioned reads stay page-aligned.
...
/// it from `PYRAMID_RESIDENT_BYTES` via `xy_tile_budget_set` (WP2 wires the
