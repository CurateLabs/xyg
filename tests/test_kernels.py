...
def test_size_sentinel_detection_matches_platform_usize_width(monkeypatch):
    ...
    x = np.arange(8.0)
    cases = [
        ("xyg_zone_maps", lambda: _native.zone_maps(x, 4)),
        ("xyg_m4_indices", lambda: _native.m4_indices(x, x, 0.0, 8.0, 2)),
        ("xyg_bin_2d_indices", lambda: _native.bin_2d_indices(x, x, 0.0, 8.0, 0.0, 8.0, 4, 4)),
        ("xyg_histogram_uniform", lambda: _native.histogram_uniform(x, 0.0, 8.0, 4)),
        ("xyg_range_indices", lambda: _native.range_indices(x, x, 0.0, 8.0, 0.0, 8.0)),
    ]
