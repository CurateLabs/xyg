"""Cross-host label_codes_first_seen parity: Python vs Node."""
...
FIXTURE = ROOT / "tests" / "fixtures" / "label_codes_first_seen_cross_host.json"
NODE_SCRIPT = ROOT / "packages" / "xy-node" / "scripts" / "label_codes_first_seen_cross_host.mjs"
...
def _python_results() -> dict[str, dict]:
    ...
    for spec in fixture["cases"]:
        categories, codes = kernels.label_codes_first_seen(spec["labels"])
        out[spec["name"]] = {
...
def test_label_codes_first_seen_cross_host(name: str, node_results: dict[str, dict]) -> None:
