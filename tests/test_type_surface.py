...
def test_component_types_are_lazy_public_root_exports() -> None:
    for name in (
        ...
        assert name in xyg.__all__
        assert getattr(xyg, name) is getattr(components, name)


def test_export_engine_is_lazy_public_enum() -> None:
    assert "Engine" in xyg.__all__
    assert xyg.Engine is Engine
...
def test_chart_dom_slots_are_public_styling_contract() -> None:
    ...
    assert xyg.CHART_DOM_SLOTS is components.CHART_DOM_SLOTS
...
def test_chart_class_names_are_limited_to_public_dom_slots() -> None:
    chart = xyg.chart(
        ...
        class_names={slot: f"slot-{slot}" for slot in xyg.CHART_DOM_SLOTS},
    )

    assert chart.class_names["legend"] == "slot-legend"
    with pytest.raises(ValueError, match="unknown slot"):
        xyg.chart(xyg.scatter(x=[1.0], y=[2.0]), class_names={"plot": "not-a-slot"})


def test_components_module_all_matches_root_component_exports() -> None:
    component_exports = {
        name for name, module_name in xyg._EXPORTS.items() if module_name == ".components"
    }
...
def test_composition_alpha_contract_is_explicitly_exported() -> None:
    ...
    for name in sorted(contract):
        ...
        assert xyg._EXPORTS[name] == ".components"
        assert getattr(xyg, name) is getattr(components, name)
